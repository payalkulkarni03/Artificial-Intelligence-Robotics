...
static int moveI,moveJ; //board indexes of current AI
moves - used in printBoard()
...
public static void selectSymbol(){ //Select
Symbol (X | O)
String input = "";
...
if(moveCount!=0)
{
System.out.println("AI Player marked "+aiPlayer+" at position
"+(moveI+1)+"-"+(moveJ+1));
...
public static boolean validMove(int boardNumber) //Check if position on
board is empty
{
...
public static boolean isHumanTurn(){ //Check if its
human's turn
if(moveCount % 2 == 0)
return true;
return false;
...
int val = getBoardValue(board,0,false); //send new
board to getBoardValue
board[i][j] = " "; //undo move
if(val > bestValue)
{
bestValue = val;
...
return maxValue;
} else //if score is to be minimized :
in case of Human Player
{
int minValue = 10000;
for(int i=0;i<3;i++)
for(int j=0;j<3;j++)
...
public static String checkWin(String newBoard[][]){ //Accepts a board and Checks
if User/AI has won, or draw
//returns win or draw or nothing
for(int i=0;i<3;i++) //checking rows & cols
{
if(newBoard[i][0].equals(newBoard[i][1]) && newBoard[i]
[1].equals(newBoard[i][2]))
{
...
if(newBoard[0][i].equals(newBoard[1][i]) && newBoard[1]
[i].equals(newBoard[2][i]))
{
...
if((newBoard[0][0].equals(newBoard[1][1]) && newBoard[1]
[1].equals(newBoard[2][2])) || (newBoard[0][2].equals(newBoard[1][1]) && newBoard[1]
[1].equals(newBoard[2][0])))
{
...
public static void aiMove(){ //calls
findBestMove()
findBestMove();
}
public static void play(){ //keep playing
while value returned from checkWin() is "nothing"
moveCount = 0;
printBoard();
